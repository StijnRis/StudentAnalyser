...
def add_learning_goals_in_error_ai_detection(learning_goals: list[LearningGoal]):
    def add_learning_goals_in_error_ai_detection(data: Dict[str, pd.DataFrame]) -> None:
        ...
        Adds a column 'learning_goals_in_error_ai_detection' with the detected learning goal(s) (or None if not detected).
        """
        execution_errors_df = data["execution_errors"]
        executions_df = data["executions"]
        file_versions_df = data["file_versions"]
        ...
        merged = chatbot.add_column_through_chatbot(
            merged,
            column_name="learning_goals_in_error_ai_detection",
            prompt_fn=prompt_fn,
            extract_fn=extract_fn,
            max_retries=3,
        )
        
        execution_errors_df["learning_goals_in_error_ai_detection"] = merged[
            "learning_goals_in_error_ai_detection"
        ]
        execution_errors_df["learning_goals_in_error_ai_detection_prompt"] = merged[
            "learning_goals_in_error_ai_detection_prompt"
        ]
        execution_errors_df["learning_goals_in_error_ai_detection_response"] = merged[
            "learning_goals_in_error_ai_detection_response"
        ]
